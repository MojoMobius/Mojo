 ...
 */
class GetjobhooviewController extends AppController {
...
//                        $col["editrules"] = array("custom"=>true,"custom_func"=>"function(val,label){return mapping(val,label);}"); 
//                        
//=======
//                        $col["editoptions"] = array("value" => $opt);
//                        $col["editrules"] = array("custom" => true, "custom_func" => "function(val,label){return mapping(val,label);}");
...
//            $g->set_columns($cols);
            ...
            $out = $g->render("list1");
            $this->set('out', $out);
            $this->render('/Getjobhooview/index_vertical');
            //       echo $out;
        ...
        if (isset($this->request->data['clicktoviewPre'])) {
            ...
            $this->redirect(array('controller' => 'Getjobhooview', 'action' => 'index/'.$InputEntityId.'/'.$page));
        }
        ...
        $InprogressProductionjob = $connection->execute("SELECT $tempFileds TotalTimeTaken,Id,[1383],ProjectId,RegionId,InputEntityId,SequenceNumber,StatusId FROM ML_ProductionEntityMaster WHERE InputEntityId=" . $InputEntityId)->fetchAll('assoc');
        ...
        if (isset($productionjobNew)) {
            ...
            //$SequenceNumber = $connection->execute('SELECT ' . $tempFileds . 'TotalTimeTaken,Id,ProjectId,RegionId,InputEntityId,SequenceNumber,StatusId FROM ML_ProductionEntityMaster WHERE InputEntityId=' . $ProdInputEntity)->fetchAll('assoc');
