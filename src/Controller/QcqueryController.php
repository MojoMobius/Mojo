 ...
 */
class QcqueryController extends AppController {
    ...
    public function index() {
        ...
        $ProList = $this->Qcquery->find('GetMojoProjectNameList', ['proId' => $is_project_mapped_to_user]);
        ...
        if (isset($this->request->data['ProjectId']) || isset($this->request->data['RegionId'])) {
            $region = $this->Qcquery->find('region', ['ProjectId' => $this->request->data['ProjectId'], 'RegionId' => $this->request->data['RegionId'], 'SetIfOneRow' => 'yes']);
        ...
        if (isset($this->request->data['UserGroupId'])) {
            $UserGroup = $this->Qcquery->find('usergroupdetails', ['ProjectId' => $_POST['ProjectId'], 'RegionId' => $_POST['RegionId'], 'UserId' => $session->read('user_id'), 'UserGroupId' => $this->request->data['UserGroupId']]);
        ...
        if (isset($this->request->data['ProjectId']) || isset($this->request->data['ModuleId'])) {
            $Modules = $this->Qcquery->find('module', ['ProjectId' => $this->request->data['ProjectId'], 'ModuleId' => $this->request->data['ModuleId']]);
        ...
        if (isset($this->request->data['check_submit']) || isset($this->request->data['formSubmit'])) {
            ...
            $user_id_list = $this->Qcquery->find('resourceDetailsArrayOnly', ['ProjectId' => $_POST['ProjectId'], 'RegionId' => $_POST['RegionId'], 'UserId' => $session->read('user_id'), 'UserGroupId' => $this->request->data['UserGroupId']]);
            ...
            if ($user_id != '') {
                $conditions.="  AND ME_QcUserQuery.UserID in (" . implode(',', $user_id) . ")";
            }
//            if ($user_group_id != '') {
//                $conditions.="  AND UserGroupId in (" . $user_group_id . ")";
//            }
            if ($ModuleId != 0) {
                $conditions.="  AND ME_QcUserQuery.ModuleId = " . $ModuleId . "";
            }
            $moduleTable = 'Staging_' . $ModuleId . '_Data';
            $queryData = $connection->execute("SELECT distinct Tdata.AttributeValue as domainID,Tdata.InputEntityId, ME_QcUserQuery.ProductionEntityId,ME_QcUserQuery.ModuleId,ME_QcUserQuery.Id ,TLComments,QueryRaisedDate, ME_QcUserQuery.UserID,ME_QcUserQuery.Query FROM ME_QcUserQuery INNER JOIN MC_CengageProcessInputData as Tdata ON Tdata.ProductionEntityID=ME_QcUserQuery.ProductionEntityId"
                            . " WHERE Tdata.AttributeMasterId=" . $domainId . " AND ME_QcUserQuery.ProjectId=" . $ProjectId . " AND ME_QcUserQuery.StatusID in (1,2,4) " . $conditions)->fetchAll('assoc');
            $i = 0;
    ...
    function ajaxregion() {
        echo $region = $this->Qcquery->find('region', ['ProjectId' => $_POST['projectId']]);
        exit;
    }

    function ajaxfilelist() {
        echo $file = $this->Qcquery->find('filelist');
        exit;
    }

    function ajaxstatus() {
        echo $file = $this->Qcquery->find('status', ['ProjectId' => $_POST['projectId'], 'importType' => $_POST['importType']]);
        exit;
    }

    function ajaxmodule() {
        echo $module = $this->Qcquery->find('module', ['ProjectId' => $_POST['ProjectId'], 'RegionId' => $_POST['RegionId'], 'ModuleId' => $ModuleId]);
        exit;
    }

    function getusergroupdetails() {
        $session = $this->request->session();
        echo $module = $this->Qcquery->find('usergroupdetails', ['ProjectId' => $_POST['projectId'], 'RegionId' => $_POST['regionId'], 'UserId' => $session->read('user_id')]);
        exit;
    }

    function getresourcedetails() {
        $session = $this->request->session();
        echo $module = $this->Qcquery->find('resourcedetails', ['ProjectId' => $_POST['projectId'], 'RegionId' => $_POST['regionId'], 'UserGroupId' => $_POST['userGroupId']]);
        exit;
    }

    public function delete($id = null) {
        $Qcquery = $this->Qcquery->get($id);
        if ($id) {
            $user_id = $this->request->session()->read('user_id');
            $Qcquery = $this->Qcquery->patchEntity($Qcquery, ['ModifiedBy' => $user_id, 'ModifiedDate' => date("Y-m-d H:i:s"), 'RecordStatus' => 0]);
            if ($this->Qcquery->save($Qcquery)) {
                $this->Flash->success(__('Import Initiate deleted Successfully'));
                return $this->redirect(['action' => 'index']);
            }
        }
        $this->set('Qcquery', $Qcquery);
        $this->render('index');
    }
    ...
    public function ajaxqueryinsert() {
        ...
        $ProjectId = $session->read("ProjectId");
        $UpdateQryStatus = "update ME_QcUserQuery set  TLComments='" . trim($_POST['mobiusComment']) . "' ,StatusID='" . $_POST['status'] . "' ,ModifiedBy=$user,ModifiedDate='" . date('Y-m-d H:i:s') . "' where Id='" . $_POST['queryID'] . "' ";
        ...
        if ($_POST['status'] == 3) {
          ...
          //  $first_Status_name = $JsonArray['ModuleStatusList'][$_POST['ModuleId']][0];
            $first_Status_name =  "Ready for QC";
