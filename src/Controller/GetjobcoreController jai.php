...
//                    $completion_status = $queryStatusId;
            ...
            $InprogressProductionjob = $connection->execute('SELECT  * FROM ' . $stagingTable . ' WITH (NOLOCK) WHERE StatusId=' . $next_status_id . '  AND ProjectId=' . $ProjectId . ' AND UserId= ' . $user_id)->fetchAll('assoc');
            if (empty($InprogressProductionjob)) {
                $productionjob = $connection->execute('SELECT TOP 1 * FROM ' . $stagingTable . ' WITH (NOLOCK) WHERE StatusId=' . $first_Status_id . '  AND ProjectId=' . $ProjectId)->fetchAll('assoc');
                if (empty($productionjob)) {
                    $this->set('NoNewJob', 'NoNewJob');
                } else {
                    if ($productionjob[0]['StatusId'] == $first_Status_id && ($newJob == 'NewJob' || $newJob == 'newjob')) {
    ...
    function ajaxgetgroupurl() {
       ...
       if($AttrId != ''){
        ...
        $sameUrl = $getData['attrinitiallink'];
            ...
            //$sameIdlink=$connection->execute("execute dbo.spSearchStringInTable @SearchString = N'$sameUrl', @table_schema = 'dbo', @table_name = 'Staging_6694018_Data'");
        ...
        if (empty($this->request->session()->read('user_id'))) {
            ...
            if (isset($insert)) {
                ...
                foreach($seqVal as $depKey=>$depVal){
                  ...
                  $updateTable=$connection->execute("INSERT INTO Staging_".$moduleId."_Data ($insertcolumn UserId,DependencyTypeMasterId,SequenceNumber,ProjectId,RegionId,InputEntityId,StatusId,ProductionEntity) values($insertFields $user_id,$depKey,$seqKey,$ProjectId,$RegionId,$InputEntityId,$next_status_id,$ProductionEntityID)");    
                }
            }
