 ...
 */
class QctlrebuttallistController extends AppController {
    ...
    public function index() {
        ...
        foreach ($module as $key => $value) {
            ...
            if (!empty($proFieldssarr))
                $ProductionFields = array_merge($ProductionFields, $proFieldssarr);
...
//                $InputEntityId = $connection->execute('SELECT TOP 1 * FROM ' . $stagingTable . ' WITH (NOLOCK) WHERE StatusId=' . $next_status_id . ' AND SequenceNumber=' . $page . ' AND ProjectId=' . $ProjectId . ' AND UserId= ' . $user_id)->fetchAll('assoc');
           ...
           $QcErrorComments[$CengageProcessInputData[$key]['AttributeMasterId']]['seq'] = $this->Qctlrebuttallist->ajax_GetQcComments_seq($CengageProcessInputData[$key]['InputEntityId'], $CengageProcessInputData[$key]['AttributeMasterId'], $CengageProcessInputData[$key]['ProjectAttributeMasterId'], 1);
            }
