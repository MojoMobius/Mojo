 ...
 */
class GetjoburlproductionController extends AppController {
    ...
    public function index() {
        ...
        if ($frameType == 1) {
            ...
            $InprogressProductionjob = $connection->execute('SELECT * FROM Staging_1153_Data WITH (NOLOCK) WHERE UserId=' . $user_id . ' AND StatusId=' . $next_status_id . ' AND SequenceNumber=1 AND ProjectId=' . $ProjectId)->fetchAll('assoc');
            if (empty($InprogressProductionjob)) {
                $productionjob = $connection->execute('SELECT TOP 1 * FROM Staging_1153_Data WITH (NOLOCK) WHERE StatusId=' . $first_Status_id . ' AND SequenceNumber=1,ProjectId=' . $ProjectId)->fetchAll('assoc');
                if (empty($productionjob)) {
                    ...
                    foreach ($productionjob as $val) {
                        if ($val['StatusId'] == $first_Status_id && ($newJob == 'NewJob' || $newJob == 'newjob')) {
                            if ($this->Getjoburlproduction->updateAll(['StatusId' => $next_status_id, 'UserId' => $user_id, 'ActStartDate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $val['ProductionEntity']])) {
...
//                $this->set('FirstLink', $FirstLink);
            ...
            if (isset($this->request->data['Submit'])) {
                ...
                if (count($DynamicFields) > 1) {
                    ...
                    $Dynamicproductionjob = $connection->execute('UPDATE Staging_1153_Data SET ' . $dymamicupdatetempFileds . 'where ProductionEntity=' . $ProductionEntity);
                }

                $queryStatus = $connection->execute("SELECT count(1) as cnt FROM ME_UserQuery WITH (NOLOCK) WHERE ProjectId=" . $ProjectId . " AND  InputEntityId='" . $productionjobNew['ProductionEntity'] . "'")->fetchAll('assoc');
                ...
                if ($this->Getjoburlproduction->updateAll(['StatusId' => $completion_status, 'ActEnddate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $ProductionEntity])) {
                    ...
                    $this->redirect(array('controller' => 'Getjoburlproduction', 'action' => '', '?' => array('job' => $submitType)));
            ...
            //$grid["scroll"] = true; 
            $g->set_options($grid);
            $e["on_insert"] = array("insert_data", new GetjoburlproductionController(), false);
            $e["on_update"] = array("update_data", new GetjoburlproductionController(), false);
            $g->set_events($e);
            ...
            $temp = rtrim($temp, ',');
            $g->select_command = "SELECT Id," . $temp . " FROM Staging_1153_Data WITH (NOLOCK) where StatusId=" . $next_status_id;
            ...
            $g->table = "Staging_1153_Data";
            $out = $g->render("list1");
            $this->set('out', $out);
            $this->render('/Getjoburlproduction/index_vertical');
            //       echo $out;
            ...
            if (isset($this->request->data['clicktoviewPre'])) {
                ...
                $this->redirect(array('controller' => 'Getjoburlproduction', 'action' => 'index/' . $page));
            }
            ...
            if (isset($this->request->data['DeleteVessel'])) {
                ...
                if ($sequence == 1) {
                    $SequenceNumber = $connection->execute('SELECT ' . $tempFileds . 'TimeTaken,Id,BatchID,BatchCreated,ProjectId,RegionId,InputEntityId,ProductionEntity,SequenceNumber,StatusId,UserId FROM Staging_1153_Data WITH (NOLOCK) WHERE ProductionEntity=' . $ProductionEntity)->fetchAll('assoc');
                    $sequencemax = count($SequenceNumber);
                    if ($sequencemax == 1)
                        return 'Minimum one record required';
                }
                $delete = $connection->execute("DELETE FROM Staging_1153_Data WHERE   ProductionEntity='" . $ProductionEntity . "' and SequenceNumber='" . $sequence . "'");
                $SequenceNumber = $connection->execute("SELECT Id,SequenceNumber FROM Staging_1153_Data  WITH (NOLOCK) WHERE  ProductionEntity='" . $ProductionEntity . "' AND SequenceNumber>$sequence order by SequenceNumber desc")->fetchAll('assoc');
                ...
                foreach ($SequenceNumber as $key => $val) {
                    ...
                    $id = $val['Id'];
                    // echo "update  Staging_1153_Data set SequenceNumber = $newsequence WHERE Id=".$val['Id']."  and SequenceNumber='".$val['SequenceNumber']."'"; exit;
                    $update = $connection->execute("update  Staging_1153_Data set SequenceNumber = $newsequence WHERE Id=" . $val['Id'] . "  and SequenceNumber='" . $val['SequenceNumber'] . "'");
                ...
                $this->redirect(array('controller' => 'Getjoburlproduction', 'action' => 'index/'));
            }
            ...
            $InprogressProductionjob = $connection->execute('SELECT TOP 1 * FROM Staging_1153_Data WITH (NOLOCK) WHERE StatusId=' . $next_status_id . ' AND SequenceNumber=' . $page . ' AND ProjectId=' . $ProjectId . ' AND UserId= ' . $user_id)->fetchAll('assoc');
            if (empty($InprogressProductionjob)) {
                $productionjob = $connection->execute('SELECT TOP 1 * FROM Staging_1153_Data WITH (NOLOCK) WHERE StatusId=' . $first_Status_id . ' AND SequenceNumber=' . $page . ' AND ProjectId=' . $ProjectId)->fetchAll('assoc');
                if (empty($productionjob)) {
                    $this->set('NoNewJob', 'NoNewJob');
                } else {
                    //echo $newJob;
                    ...
                    if ($productionjob[0]['StatusId'] == $first_Status_id && ($newJob == 'NewJob' || $newJob == 'newjob')) {
                        if ($this->Getjoburlproduction->updateAll(['StatusId' => $next_status_id, 'UserId' => $user_id, 'ActStartDate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $productionjob[0]['ProductionEntity']])) {
           ...
           // pr($StaticFields);
            ...
            //pr($productionjobNew);
            if (isset($productionjobNew)) {
                $SequenceNumber = $connection->execute('SELECT ' . $tempFileds . 'TimeTaken,Id,BatchID,BatchCreated,ProjectId,RegionId,InputEntityId,ProductionEntity,SequenceNumber,StatusId,UserId FROM Staging_1153_Data WITH (NOLOCK)  WHERE ProductionEntity=' . $productionjobNew['ProductionEntity'] . ' ORDER BY SequenceNumber')->fetchAll('assoc');
                ...
                $this->set('TimeTaken', $TimeTaken);
                $link = $connection->execute("SELECT Id,DomainUrl,DownloadStatus,InputId FROM ME_DomainUrl WITH (NOLOCK) WHERE   ProjectId=" . $ProjectId . " AND RegionId=" . $productionjobNew['RegionId'] . " AND DomainId='" . $DomainIdName . "'")->fetchAll('assoc');
                //   pr($link); 
            ...
            if (isset($this->request->data['Submit'])) {
                ...
                if ($this->Getjoburlproduction->updateAll(['StatusId' => $completion_status, 'ActEnddate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $ProductionEntity])) {
                    //$productionjob = $connection->execute('INSERT INTO  ME_Production_TimeMetric( ProjectId,ProductionEntityID,InputEntityId,Module_Id,Start_Date,End_Date,TimeTaken,UserId,' . $updatetempFileds . ' )values ( ' . $productionjobNew['BatchID'] . ',' . $productionjobNew['ProjectId'] . ',' . $productionjobNew['RegionId'] . ',' . $productionjobNew['InputEntityId'] . ',' . $productionjobNew['ProductionEntity'] . ',' . $SequenceNumber . ',' . $productionjobNew['StatusId'] . ',' . $productionjobNew['StatusId'] . ',' . $valuetoInsert . ')');
                    $productionjob = $connection->execute("UPDATE ProductionEntityMaster SET StatusId=" . $completion_status . ",ProductionEndDate='" . date('Y-m-d H:i:s') . "' WHERE ID=" . $ProductionEntity);
                    $updateUrlMonitoring = $connection->execute("UPDATE ME_Url_Monitoring_".$productionjobNew['ProjectId']." SET Status='3' WHERE InputEntityId=".$productionjobNew['InputEntityId']);
                    $this->redirect(array('controller' => 'Getjoburlproduction', 'action' => '', '?' => array('job' => $submitType)));
...
//                                $this->Flash->success(__('Query posted Successfully'));
            ...
            //$GetUrlRemarks = $this->UrlMonitoring->Get_UrlRemarks();
    ...
    public function update_data($data) {
        ...
        $g->execute_query("UPDATE Staging_1153_Data SET " . $temp . " ,RecordStatus=1 WHERE Id = " . $data['Id'] . "");
    }
    ...
    public function insert_data($data) {
        ...
        $tempFields = '';
        $tempData = '';
        $InprogressProductionjob = $connection->execute('SELECT TOP 1 BatchID,BatchCreated,ProjectId,RegionId,InputEntityId,ProductionEntity,SequenceNumber,StatusId,UserId,ActStartDate FROM Staging_1153_Data WITH (NOLOCK) WHERE UserId=' . $user_id . ' AND StatusId=' . $next_status_id . ' ORDER BY SequenceNumber DESC')->fetchAll('assoc');
        foreach ($ProductionFields as $key => $val) {
            if ($data['params'][$val['AttributeName']] != '') {
    ...
    function ajaxqueryposing() {
        ...
        echo $_POST['query'];
        $file = $this->Getjoburlproduction->find('querypost', ['ProductionEntity' => $_POST['InputEntyId'], 'query' => $_POST['query'], 'ProjectId' => $ProjectId, 'moduleId' => $moduleId, 'user' => $user_id]);
        exit;
    }
    ...
    public function ajaxsave() {
        ...
        if (empty($this->request->session()->read('user_id'))) {
            ...
            $productionjob = $connection->execute('UPDATE Staging_1153_Data SET ' . $updatetempFileds . 'where ProductionEntity=' . $_POST['ProductionEntity'] . ' AND SequenceNumber=' . $_POST['SequenceNumber']);
            $Dynamicproductionjob = $connection->execute('UPDATE Staging_1153_Data SET ' . $dymamicupdatetempFileds . 'where ProductionEntity=' . $_POST['ProductionEntity']);
            echo 'saved';
            exit;
        }
    }
    ...
    public function ajaxgetnextpagedata() {
        if (empty($this->request->session()->read('user_id'))) {
            ...
            $connection = ConnectionManager::get('default');
            // echo 'SELECT BatchID,ProjectId,RegionId,InputEntityId,ProductionEntity,StatusId,UserId FROM Staging_1153_Data WHERE ProductionEntity='.$_POST['ProductionEntity'];
            $productionjobNew = $connection->execute('SELECT * FROM Staging_1153_Data WITH (NOLOCK) WHERE ProductionEntity=' . $_POST['ProductionEntity'] . ' AND SequenceNumber=' . $_POST['page'])->fetchAll('assoc');
            //pr($productionjobNew);
    ...
    public function ajaxnewsave() {
        if (empty($this->request->session()->read('user_id'))) {
            ...
            $Dynamicproductionjob = $connection->execute('UPDATE Staging_1153_Data SET ' . $dymamicupdatetempFileds . 'where ProductionEntity=' . $refData['ProductionEntity']);
            echo 'saved';
            exit;
        }
    }
    ...
    function ajaxdelete() {
        if (empty($this->request->session()->read('user_id'))) {
            ...
            if ($sequence == 1) {
                $SequenceNumber = $connection->execute('SELECT Id FROM Staging_1153_Data WITH (NOLOCK) WHERE ProductionEntity=' . $ProductionEntity)->fetchAll('assoc');
                $sequencemax = count($SequenceNumber);
                if ($sequencemax == 1)
                    $delete = 'No';
            }
            if ($delete != 'No') {
                // echo "DELETE FROM Staging_1153_Data WHERE   ProductionEntity='" . $ProductionEntity . "' and SequenceNumber='" . $sequence . "'";
                $delete = $connection->execute("DELETE FROM Staging_1153_Data WITH (NOLOCK) WHERE   ProductionEntity='" . $ProductionEntity . "' and SequenceNumber='" . $sequence . "'");
                $SequenceNumber = $connection->execute("SELECT Id,SequenceNumber FROM Staging_1153_Data with (NOLOCK)  WHERE  ProductionEntity='" . $ProductionEntity . "' AND SequenceNumber>$sequence order by SequenceNumber desc")->fetchAll('assoc');
                foreach ($SequenceNumber as $key => $val) {
                    $newsequence = $val['SequenceNumber'] - 1;
                    $id = $val['Id'];
...
//        
//    //    echo $html->link('Demo',array('controller'=>'posts','action'=>'add'),array('class'=>'popupButton', 'onclick' => 'false')); 
//        
//        $this->layout = 'ajax';
//        error_reporting(E_PARSE);
//        $connection = ConnectionManager::get('default');
//        $val = $this->GetInvalidView->find('GetInvalidView',$_POST);
//        //$val = $this->render('/GetInvalidView/index');
//        echo $val;
//        //echo $val['DomainUrlId'];
//        exit;
//    }
//    
...
//        $user_id = $session->read("user_id");
//        $ProjectId = $session->read("ProjectId");
//        //$val = $this->GetInvalidView->find('GetInvalidDomainUrl',['ProductionEntity' => $_POST['InputEntyId'], 'query' => $_POST['query'], 'ProjectId' => $ProjectId, 'moduleId' => $moduleId, 'user' => $user_id]);
//        $val = $this->GetInvalidView->find('GetInvalidDomainUrl',['DomainId' => $_POST['DomainId'], 'DomainUrl' => $_POST['DomainUrl'], 'ProjectId' => $ProjectId, 'RegionId' => $_POST['RegionId'], 'user' => $user_id]);
