 ...
 */
class GetjobnoncoreController extends AppController {
    ...
    public function index() {
        ...
        if ($frameType == 1) {
            ...
            $InprogressProductionjob = $connection->execute('SELECT * FROM Staging_1150_Data WITH (NOLOCK) WHERE UserId=' . $user_id . ' AND StatusId=' . $next_status_id . ' AND SequenceNumber=1 AND ProjectId=' . $ProjectId)->fetchAll('assoc');
            if (empty($InprogressProductionjob)) {
                $productionjob = $connection->execute('SELECT TOP 1 * FROM Staging_1150_Data WITH (NOLOCK) WHERE StatusId=' . $first_Status_id . ' AND SequenceNumber=1 AND ProjectId=' . $ProjectId)->fetchAll('assoc');
                if (empty($productionjob)) {
                    ...
                    foreach ($productionjob as $val) {
                        if ($val['StatusId'] == $first_Status_id && ($newJob == 'NewJob' || $newJob == 'newjob')) {
                            if ($this->Getjobnoncore->updateAll(['StatusId' => $next_status_id, 'UserId' => $user_id, 'ActStartDate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $val['ProductionEntity']])) {
            ...
            if (isset($this->request->data['Submit'])) {
                ...
                if (count($DynamicFields) > 1) {
                    ...
                    $Dynamicproductionjob = $connection->execute('UPDATE Staging_1150_Data SET ' . $dymamicupdatetempFileds . 'where ProductionEntity=' . $ProductionEntity);
                }
                ...
                if ($this->Getjobnoncore->updateAll(['StatusId' => $completion_status, 'ActEnddate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $ProductionEntity])) {
                    ...
                    $this->redirect(array('controller' => 'Getjobnoncore', 'action' => '', '?' => array('job' => $submitType)));
            ...
            //$grid["scroll"] = true; 
            $g->set_options($grid);
            $e["on_insert"] = array("insert_data", new GetjobnoncoreController(), false);
            $e["on_update"] = array("update_data", new GetjobnoncoreController(), false);
            $g->set_events($e);
            ...
            $temp = rtrim($temp, ',');
            $g->select_command = "SELECT Id," . $temp . " FROM Staging_1150_Data WITH (NOLOCK) where StatusId=" . $next_status_id;
            ...
            $g->table = "Staging_1150_Data";
            $out = $g->render("list1");
            $this->set('out', $out);
            $this->render('/Getjobnoncore/index_vertical');
            //       echo $out;
            ...
            if (isset($this->request->data['clicktoviewPre'])) {
                ...
                $this->redirect(array('controller' => 'Getjobnoncore', 'action' => 'index/' . $page));
            }
            ...
            if (isset($this->request->data['DeleteVessel'])) {
                ...
                if ($sequence == 1) {
                    $SequenceNumber = $connection->execute('SELECT ' . $tempFileds . 'TimeTaken,Id,BatchID,BatchCreated,ProjectId,RegionId,InputEntityId,ProductionEntity,SequenceNumber,StatusId,UserId FROM Staging_1150_Data WITH (NOLOCK) WHERE ProductionEntity=' . $ProductionEntity)->fetchAll('assoc');
                    $sequencemax = count($SequenceNumber);
                    if ($sequencemax == 1)
                        return 'Minimum one record required';
                }
                $delete = $connection->execute("DELETE FROM Staging_1150_Data WHERE   ProductionEntity='" . $ProductionEntity . "' and SequenceNumber='" . $sequence . "'");
                $SequenceNumber = $connection->execute("SELECT Id,SequenceNumber FROM Staging_1150_Data  WITH (NOLOCK) WHERE  ProductionEntity='" . $ProductionEntity . "' AND SequenceNumber>$sequence order by SequenceNumber desc")->fetchAll('assoc');
                ...
                foreach ($SequenceNumber as $key => $val) {
                    ...
                    $id = $val['Id'];
                    // echo "update  Staging_1150_Data set SequenceNumber = $newsequence WHERE Id=".$val['Id']."  and SequenceNumber='".$val['SequenceNumber']."'"; exit;
                    $update = $connection->execute("update  Staging_1150_Data set SequenceNumber = $newsequence WHERE Id=" . $val['Id'] . "  and SequenceNumber='" . $val['SequenceNumber'] . "'");
                ...
                $this->redirect(array('controller' => 'Getjobnoncore', 'action' => 'index/'));
            }
            ...
            $InprogressProductionjob = $connection->execute('SELECT TOP 1 * FROM Staging_1150_Data WITH (NOLOCK) WHERE StatusId=' . $next_status_id . ' AND SequenceNumber=' . $page . ' AND ProjectId=' . $ProjectId . ' AND UserId= ' . $user_id)->fetchAll('assoc');
            if (empty($InprogressProductionjob)) {
                $productionjob = $connection->execute('SELECT TOP 1 * FROM Staging_1150_Data WITH (NOLOCK) WHERE StatusId=' . $first_Status_id . ' AND SequenceNumber=' . $page . ' AND ProjectId=' . $ProjectId)->fetchAll('assoc');
                if (empty($productionjob)) {
                    $this->set('NoNewJob', 'NoNewJob');
                } else {
                    //echo $newJob;
                    ...
                    if ($productionjob[0]['StatusId'] == $first_Status_id && ($newJob == 'NewJob' || $newJob == 'newjob')) {
                        if ($this->Getjobnoncore->updateAll(['StatusId' => $next_status_id, 'UserId' => $user_id, 'ActStartDate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $productionjob[0]['ProductionEntity']])) {
            ...
            if (isset($productionjobNew)) {
                $SequenceNumber = $connection->execute('SELECT ' . $tempFileds . 'TimeTaken,Id,[' . $domainId . '],BatchID,BatchCreated,ProjectId,RegionId,InputEntityId,ProductionEntity,SequenceNumber,StatusId,UserId FROM Staging_1150_Data WHERE ProductionEntity=' . $productionjobNew['ProductionEntity'])->fetchAll('assoc');
    ...
    public function update_data($data) {
        ...
        $g->execute_query("UPDATE Staging_1150_Data SET " . $temp . " ,RecordStatus=1 WHERE Id = " . $data['Id'] . "");
    }
    ...
    public function insert_data($data) {
        ...
        $tempFields = '';
        $tempData = '';
        $InprogressProductionjob = $connection->execute('SELECT TOP 1 BatchID,BatchCreated,ProjectId,RegionId,InputEntityId,ProductionEntity,SequenceNumber,StatusId,UserId,ActStartDate FROM Staging_1150_Data WITH (NOLOCK) WHERE UserId=' . $user_id . ' AND StatusId=' . $next_status_id . ' ORDER BY SequenceNumber DESC')->fetchAll('assoc');
        foreach ($ProductionFields as $key => $val) {
            if ($data['params'][$val['AttributeName']] != '') {
    ...
    function ajaxqueryposing() {
        ...
        echo $_POST['query'];
        $file = $this->Getjobnoncore->find('querypost', ['ProductionEntity' => $_POST['InputEntyId'], 'query' => $_POST['query'], 'ProjectId' => $ProjectId, 'moduleId' => $moduleId, 'user' => $user_id]);
        exit;
    }
    ...
    public function ajaxsave() {
        ...
        if (empty($this->request->session()->read('user_id'))) {
            ...
            $productionjob = $connection->execute('UPDATE Staging_1150_Data SET ' . $updatetempFileds . 'where ProductionEntity=' . $_POST['ProductionEntity'] . ' AND SequenceNumber=' . $_POST['SequenceNumber']);
            $Dynamicproductionjob = $connection->execute('UPDATE Staging_1150_Data SET ' . $dymamicupdatetempFileds . 'where ProductionEntity=' . $_POST['ProductionEntity']);
            echo 'saved';
            exit;
        }
    }
    ...
    public function ajaxgetnextpagedata() {
        if (empty($this->request->session()->read('user_id'))) {
            ...
            $connection = ConnectionManager::get('default');
            // echo 'SELECT BatchID,ProjectId,RegionId,InputEntityId,ProductionEntity,StatusId,UserId FROM Staging_1150_Data WHERE ProductionEntity='.$_POST['ProductionEntity'];
            $productionjobNew = $connection->execute('SELECT * FROM Staging_1150_Data WITH (NOLOCK) WHERE ProductionEntity=' . $_POST['ProductionEntity'] . ' AND SequenceNumber=' . $_POST['page'])->fetchAll('assoc');
            //pr($productionjobNew);
    ...
    public function ajaxnewsave() {
        if (empty($this->request->session()->read('user_id'))) {
            ...
            $Dynamicproductionjob = $connection->execute('UPDATE Staging_1150_Data SET ' . $dymamicupdatetempFileds . 'where ProductionEntity=' . $refData['ProductionEntity']);
            echo 'saved';
            exit;
        }
    }
    ...
    function ajaxdelete() {
        if (empty($this->request->session()->read('user_id'))) {
            ...
            if ($sequence == 1) {
                $SequenceNumber = $connection->execute('SELECT Id FROM Staging_1150_Data WITH (NOLOCK)WHERE ProductionEntity=' . $ProductionEntity)->fetchAll('assoc');
                $sequencemax = count($SequenceNumber);
                if ($sequencemax == 1)
                    $delete = 'No';
            }
            if ($delete != 'No') {
                // echo "DELETE FROM Staging_1150_Data WHERE   ProductionEntity='" . $ProductionEntity . "' and SequenceNumber='" . $sequence . "'";
                $delete = $connection->execute("DELETE FROM Staging_1150_Data WITH (NOLOCK) WHERE   ProductionEntity='" . $ProductionEntity . "' and SequenceNumber='" . $sequence . "'");
                $SequenceNumber = $connection->execute("SELECT Id,SequenceNumber FROM Staging_1150_Data with (NOLOCK)  WHERE  ProductionEntity='" . $ProductionEntity . "' AND SequenceNumber>$sequence order by SequenceNumber desc")->fetchAll('assoc');
                foreach ($SequenceNumber as $key => $val) {
                    $newsequence = $val['SequenceNumber'] - 1;
                    $id = $val['Id'];
