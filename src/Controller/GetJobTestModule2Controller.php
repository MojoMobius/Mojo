 ...
 */
class GetJobTestModule2Controller extends AppController {
    ...
    public function index() {
        ...
        if ($frameType == 1) {
            ...
            if (empty($InprogressProductionjob)) {
                $productionjob = $connection->execute('SELECT TOP 1 * FROM Staging_1153_Data WITH (NOLOCK) WHERE StatusId=' . $first_Status_id . ' AND SequenceNumber=1 AND ProjectId=' . $ProjectId)->fetchAll('assoc');
                if (empty($productionjob)) {
                    ...
                    foreach ($productionjob as $val) {
                        if ($val['StatusId'] == $first_Status_id && ($newJob == 'NewJob' || $newJob == 'newjob')) {
                            if ($this->GetJobTestModule2->updateAll(['StatusId' => $next_status_id, 'UserId' => $user_id, 'ActStartDate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $val['ProductionEntity']])) {
            ...
            if (isset($this->request->data['Submit'])) {
                ...
                if ($this->GetJobTestModule2->updateAll(['StatusId' => $completion_status, 'ActEnddate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $ProductionEntity])) {
                    ...
                    $this->redirect(array('controller' => 'GetJobTestModule2', 'action' => '', '?' => array('job' => $submitType)));
            ...
            //$grid["scroll"] = true; 
            $g->set_options($grid);
            $e["on_insert"] = array("insert_data", new GetJobTestModule2Controller(), false);
            $e["on_update"] = array("update_data", new GetJobTestModule2Controller(), false);
            $g->set_events($e);
            ...
            $out = $g->render("list1");
            $this->set('out', $out);
            $this->render('/GetJobTestModule2/index_vertical');
            //       echo $out;
            ...
            if (isset($this->request->data['clicktoviewPre'])) {
                ...
                $this->redirect(array('controller' => 'GetJobTestModule2', 'action' => 'index/' . $page));
            }
            ...
            if (isset($this->request->data['DeleteVessel'])) {
                ...
                $this->redirect(array('controller' => 'GetJobTestModule2', 'action' => 'index/'));
            }
            ...
            if (empty($InprogressProductionjob)) {
                ...
                if (empty($productionjob)) {
                    ...
                    if ($productionjob[0]['StatusId'] == $first_Status_id && ($newJob == 'NewJob' || $newJob == 'newjob')) {
                        if ($this->GetJobTestModule2->updateAll(['StatusId' => $next_status_id, 'UserId' => $user_id, 'ActStartDate' => date('Y-m-d H:i:s')], ['ProductionEntity' => $productionjob[0]['ProductionEntity']])) {
...
//                                $this->Flash->success(__('Query posted Successfully'));
    ...
    function ajaxqueryposing() {
        ...
        echo $_POST['query'];
        $file = $this->GetJobTestModule2->find('querypost', ['ProductionEntity' => $_POST['InputEntyId'], 'query' => $_POST['query'], 'ProjectId' => $ProjectId, 'moduleId' => $moduleId, 'user' => $user_id]);
        exit;
    }
