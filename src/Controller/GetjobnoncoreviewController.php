 ...
 */
class GetjobnoncoreviewController extends AppController {
    ...
    public function index() {
        ...
        if ($frameType == 1) {
            ...
            //$InprogressProductionjob = $connection->execute('SELECT RegionId,[1383],Id,ProductionEntity,StatusId FROM Staging_1150_Data WHERE UserId=' . $user_id . ' AND StatusId=' . $next_status_id . ' AND SequenceNumber=1')->fetchAll('assoc');
            $InprogressProductionjob = $connection->execute("SELECT * FROM ML_ProductionEntityMaster WHERE  InputEntityId = $InputEntityId ")->fetchAll('assoc');
            ...
            foreach ($ProductionFields as $key => $val) {
                if ($val['AttributeName'] != '') {
                    ...
                    if ($val['ControlName'] == 'DropDownList' && $IsAutoSuggesstion != 1) {
                        ...
                        if (empty($opt))
                            $opt['0'] = '--Select--';
                        $col["editoptions"] = array("value" => $opt);
                    }
                    if ($val['ControlName'] == 'DropDownList' && $IsAutoSuggesstion == 1) {
                        //  $col["edittype"] = "select";  
                        $opt = array();
...
//            $g->set_columns($cols);
            ...
            $out = $g->render("list1");
            $this->set('out', $out);
            $this->render('/Getjobnoncoreview/index_vertical');
            //       echo $out;
        ...
        if (isset($this->request->data['clicktoviewPre'])) {
            ...
            $this->redirect(array('controller' => 'Getjobnoncoreview', 'action' => 'index/'.$InputEntityId.'/'. $page));
        }
        ...
        $InprogressProductionjob = $connection->execute("SELECT $tempFileds TotalTimeTaken,[1383],Id,ProjectId,RegionId,InputEntityId,SequenceNumber,StatusId FROM ML_ProductionEntityMaster WHERE  InputEntityId=" . $InputEntityId)->fetchAll('assoc');
            ...
            $ReadOnlyFields = $JsonArray['ModuleAttributes'][$RegionId][$moduleId]['readonly'];
        if (isset($productionjobNew)) {
            //$SequenceNumber = $connection->execute('SELECT ' . $tempFileds . 'TotalTimeTaken,Id,1383,ProjectId,RegionId,InputEntityId,SequenceNumber,StatusId FROM ML_ProductionEntityMaster WHERE InputEntityId=' . $InputEntityId)->fetchAll('assoc');
            $SequenceNumber = $connection->execute("SELECT $tempFileds TotalTimeTaken,Id,1383,ProjectId,RegionId,InputEntityId,SequenceNumber,StatusId FROM ML_ProductionEntityMaster WHERE InputEntityId=" . $InputEntityId)->fetchAll('assoc');
