...
class DeliveryExportTable extends Table {
    ...
    public function findRegion(Query $query, array $options) {
        ...
        $template.='<select name="RegionId" id="RegionId"  class="form-control" onchange=""><option value=0>--Select--</option>';
        if (file_exists($path)) {
...
//                }
                ...
                if($IsItOkay) {
                    ...
                    $queriesR = $connection->execute("select ".$select_fields_final.",RPE.SequenceNumber,DEP.Id as DepId,DEP.FieldTypeName,RPE.InputEntityId as InputEntityId from Report_ProductionEntityMaster_" . $dt . " as RPE INNER JOIN Report_ProductionTimeMetric_" . $dt . " as RPT ON RPE.InputEntityId=RPT.InputEntityId INNER JOIN ME_Production_TimeMetric_" . $dt . " as PTM ON RPE.InputEntityId=PTM.InputEntityId INNER JOIN MC_DependencyTypeMaster as DEP ON DEP.Id=RPE.DependencyTypeMasterId where" . $options['condition'] ." group by ".$select_fields_final_group.",RPE.SequenceNumber,RPE.InputEntityId,DEP.Id,DEP.FieldTypeName order by InputEntityId");
...
//                }
                ...
                if($IsItOkay) {
                    ...
                    $queries = $connection->execute("select ".$select_fields_final.",RPE.SequenceNumber,DEP.Id as DepId,DEP.FieldTypeName,RPE.InputEntityId as InputEntityId,PTM.UserId from Report_ProductionEntityMaster_" . $dt . " as RPE "
                        ...
                        . "where RPE.BatchID = ".$options['BatchId']." AND UserId in (".$UserIdImploded.") group by ".$select_fields_final_group.",RPE.SequenceNumber,RPE.InputEntityId,DEP.Id,DEP.FieldTypeName,PTM.UserId order by InputEntityId");
