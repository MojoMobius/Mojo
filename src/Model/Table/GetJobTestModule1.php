...
class GetJobTestModule1Table extends Table {
    
    public function initialize(array $config)
    {
        $this->table('Staging_1152_Data');
        $this->primaryKey('Id');
...
//            $queryInsert = "exec InsertME_UserQuery @UserID=$user,@InputEntityId='".$InputEntyId."',@Query='".trim($query)."',@QueryRaisedDate='".date('Y-m-d H:i:s')."',@StatusID=$status,@RecordStatus=1,@CreatedDate='".date('Y-m-d H:i:s')."',@CreatedBy=$user"; 
    ...
    public function findSavedata(Query $query,array $options){
                    ...
                    //echo 'UPDATE Staging_1152_Data SET ' . $updatetempFileds . 'where Id=' . $productionjobId; exit;
                    //echo $page;
                    //echo 'UPDATE Staging_1152_Data SET ' . $updatetempFileds . 'where ProductionEntity=' . $ProductionEntity;
                    $productionjob = $connection->execute('UPDATE Staging_1152_Data SET ' . $updatetempFileds . 'where ProductionEntity=' . $ProductionEntity . ' AND SequenceNumber=' . $_POST['SequenceNumber']);
