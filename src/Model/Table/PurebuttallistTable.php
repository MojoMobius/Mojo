 ...
 * @package       app.Model
 */
class PurebuttallistTable extends Table {
   ...
   public function ajax_GetQcComments_seq($InputEntyId, $AttributeMasterId, $ProjectAttributeMasterId, $SequenceNumber) {
           ...
           $cmdOldData = $connection->execute("select mvc.QCComments,mvc.StatusID,mvc.UserReputedComments, mve.ErrorCategoryName from MV_QC_Comments as mvc inner join MV_QC_ErrorCategoryMaster as mve on mvc.ErrorCategoryMasterId = mve.Id where mvc.AttributeMasterId = $AttributeMasterId and mvc.ProjectAttributeMasterId=$ProjectAttributeMasterId and mvc.InputEntityId=$InputEntyId and mvc.StatusID IN (3) order by mvc.SequenceNumber")->fetchAll('assoc');
           if(!empty($cmdOldData)){
